...
window.title("CalcError Detector")
#function
def about():
...
def bracket_scanner():
    equ_scanner = list(inputText.get())
    print (equ_scanner)
    op_brckt = equ_scanner.count("(")
    cl_brckt = equ_scanner.count(")")
    print (op_brckt, cl_brckt)
    err_brckt = op_brckt - cl_brckt
    print (err_brckt)
    if err_brckt == 0:
        evaluation()
    elif err_brckt > 0:
        inputText.set("You missed a ) bracket ")
    elif err_brckt < 0:
        inputText.set("You missed a ( bracket ")
...
def evaluation():
    ...
    try:
         ...
         inputText.set(result)
            ...
            inputText.set("you've devided by zero!")
...
ac = Button(mainFrame, text="AC", fg="black", image=acimage, bd=0, bg="black", cursor="hand2",
               command=lambda: clearAll()).grid(row=0, column=1, padx=1, pady=1)
...
equals = Button(mainFrame, text="=", image=equalimage, fg="white", bd=0, bg="black", cursor="hand2",
                command=lambda: bracket_scanner()).grid(row=4, column=3, padx=1, pady=1)
